"""
train.py — DDP distributed training with torchrun
===================================================
Works on Windows (gloo backend, CPU or single GPU) and Linux (gloo/nccl).

Usage — single machine:
    torchrun --standalone --nproc_per_node=1 train.py [--debug]
    torchrun --standalone --nproc_per_node=2 train.py
    torchrun --standalone --nproc_per_node=4 train.py --debug

CPU-only (Windows/Linux):
    torchrun --standalone --nproc_per_node=1 train.py --no_cuda

Resume:
    torchrun --standalone --nproc_per_node=2 train.py --resume checkpoints/checkpoint_epoch_009.pth
"""
...
# ─────────────────────────────────────────────
def parse_args():
    p = argparse.ArgumentParser(description="DDP Image Recognition — ResNet-50 CIFAR-100")
    p.add_argument("--data_dir",     default="data")
    p.add_argument("--results_dir",  default="results")
    p.add_argument("--ckpt_dir",     default="checkpoints")
    p.add_argument("--epochs",       type=int,   default=30)
    p.add_argument("--batch_size",   type=int,   default=128,
                   help="Per-GPU / per-process batch size")
    p.add_argument("--lr",           type=float, default=0.1,
                   help="Base LR — code scales by world_size automatically")
    ...
    p.add_argument("--log_interval", type=int,   default=50,
                   help="Print debug log every N batches (rank 0 only)")
    p.add_argument("--debug",        action="store_true",
                   help="Extra debug output every batch and epoch")
    return p.parse_args()
...
def _choose_backend(device: torch.device) -> str:
    """
    NCCL is fastest but Linux/CUDA only.
    gloo works everywhere (Windows CPU/GPU, Linux CPU).
    """
    if device.type == "cuda" and not sys.platform.startswith("win"):
        # nccl available on Linux with CUDA
        return "nccl"
    return "gloo"
...
def build_dataloaders(args, rank, world_size):
    ...
    if rank == 0:
        print(f"  [DEBUG] Building datasets (rank 0 may trigger download) …")

    # All ranks load; rank 0 will have already created the cache dir
    ...
    if rank == 0:
        print(f"  [DEBUG] num_workers per rank: {nw}")
    ...
    train_loader = DataLoader(
        ...
        num_workers=nw,
        pin_memory=(nw > 0 and torch.cuda.is_available()),
...
def train_epoch(model, loader, criterion, optimizer, scaler,
    ...
    iterable = tqdm(loader, desc=f"Epoch {epoch}", total=len(loader)) if rank == 0 else loader
    ...
    for i, (images, targets) in enumerate(iterable):
        ...
        if rank == 0:
            ...
            if args.debug or ((i + 1) % args.log_interval == 0):
                msg = (
                    f"  [DEBUG][Train] Epoch {epoch} Batch {i+1}/{len(loader)} | "
...
def validate(model, loader, criterion, device, rank, logger, debug=False):
    ...
    # Aggregate across all ranks so every rank has the global average
    ...
    if rank == 0:
        logger.debug(f"  [DEBUG][Val] Global — "
                     f"Loss {losses.avg:.4f} | Acc@1 {top1.avg:.2f}% | Acc@5 {top5.avg:.2f}%")
...
def main():
    ...
    # Choose backend — we already inited with gloo (universal), but log the ideal
    ideal_backend = _choose_backend(device)
    actual_backend = dist.get_backend()

    # ── Dirs (rank 0 creates, then barrier so others can safely proceed) ────
    if rank == 0:
        for d in [args.data_dir, args.results_dir, args.ckpt_dir]:
            os.makedirs(d, exist_ok=True)
            print(f"  [DEBUG] Directory ready: {d}")

    dist.barrier()  # all ranks wait until dirs exist
    ...
    if rank == 0:
        ...
        print(f"  Backend     : {actual_backend}  (ideal for this hw: {ideal_backend})")
        ...
        print(f"  Base LR     : {args.lr}  "
              f"(scaled to {args.lr * world_size:.4f} for world_size={world_size})")
        print(f"  Debug       : {args.debug}")
        print("=" * 70)
        logger.info(f"DDP init complete — rank {rank}/{world_size} on {device}")
    ...
    try:
        ...
        scaled_lr = args.lr * world_size  # linear LR scaling rule
        ...
        # FIX: use non-deprecated torch.amp.GradScaler (was torch.cuda.amp.GradScaler)
        ...
        if args.resume:
            ...
            if rank == 0:
                print(f"  [DEBUG] Resumed at epoch {start_epoch}, best_acc={best_acc:.2f}%")
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if rank == 0 and args.debug:
                print(f"\n  [DEBUG] === Epoch {epoch}/{args.epochs-1} start ===")

            # Critical: set_epoch so each rank shuffles differently per epoch
            train_sampler.set_epoch(epoch)

            train_loss, train_acc1, train_acc5 = train_epoch(
                model, train_loader, criterion, optimizer,
                scaler, device, args, epoch, rank, logger,
            )
            ...
            if rank == 0:
                line = (
                    ...
                    f"LR {current_lr:.5f} | "
                    f"Train {train_loss:.4f}/{train_acc1:.2f}% | "
                    f"Val {val_loss:.4f}/{val_acc1:.2f}% | "
                    f"Best {best_acc:.2f}% | "
        ...
        # Always destroy the process group, even on crash — critical on Windows
